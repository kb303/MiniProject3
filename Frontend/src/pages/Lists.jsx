...
export default function Lists({ setSelectedMovie }) {
  ...
  useEffect(() => {
    ...
    syncLoginStatus();
    window.addEventListener("authStateChanged", syncLoginStatus);
    window.addEventListener("storage", syncLoginStatus);

    return () => {
      window.removeEventListener("authStateChanged", syncLoginStatus);
  ...
  if (!isLoggedIn) {
    return (
      <div className="max-w-[1400px] mx-auto px-6 py-24 text-center text-muted-foreground">
        <p className="text-lg">Please log in to view your lists.</p>
        <button
        ...
        {showNewList && (
          <div className="mb-4 p-3 bg-card rounded-xl border border-border">
            ...
            <div className="flex gap-2">
              <button
                onClick={createList}
                className="flex-1 text-xs py-1.5 bg-primary text-primary-foreground rounded-lg font-medium hover:bg-primary/90 transition-colors"
              >
                Create
              </button>
              <button
                ...
                className="flex-1 text-xs py-1.5 bg-muted text-muted-foreground rounded-lg hover:text-foreground transition-colors"
              >
                Cancel
              </button>
            </div>
            ...
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-muted-foreground text-sm mt-2">
                  {activeList.movieIds.length} title
                  {activeList.movieIds.length !== 1 ? "s" : ""}
                </p>
              </div>
              <div className="flex items-center gap-3 mt-1">
                {activeList.movieIds.length > 1 && (
                  <SortSelect
                    ...
                    onChange={(v) =>
                      setListSorts((prev) => ({ ...prev, [activeList.id]: v }))
                    }
                  />
                )}
                <button
            ...
            {activeList.movieIds.length === 0 ? (
              <div className="text-center py-24 border border-dashed border-border rounded-2xl text-muted-foreground">
                ...
                <p className="text-xs mt-1 opacity-60">
                  Open any movie and use "Add to List" to add it here.
                </p>
              </div>
            ) : allMovies.length === 0 ? (
              ...
              (() => {
                const listMovies = sortMovies(
                  activeList.movieIds
                    .map((id) => {
                      const resolvedId =
                        legacyMovieIdMap[String(id)] ?? String(id);
                      return allMovies.find(
                        (movie) =>
                          String(movie.id) === resolvedId ||
                          String(movie._id) === resolvedId,
                      );
                    })
                    .filter(Boolean),
                      ...
                      >
                        <div className="relative overflow-hidden rounded-xl aspect-[2/3] bg-muted">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              toggleMovieInList(activeList.id, movie.id);
                            }}
